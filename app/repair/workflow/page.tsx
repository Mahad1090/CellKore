		...
		<main className="min-h-screen bg-background">
			...
			<section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
					...
					{REPAIR_TABS.map((tab) => (
						<Link
							...
							className={`text-center px-4 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-[0.14em] transition-all ${
								tab.href === '/repair/workflow' ? 'bg-primary text-primary-foreground' : 'text-foreground/70 hover:bg-muted'
							}`}
						>
							{tab.label}
						</Link>
				...
				<div className="bg-card border border-border rounded-3xl p-8">
					<h1 className="text-2xl font-bold tracking-luxury uppercase text-foreground">Repair Workflow</h1>
					<p className="text-sm text-muted-foreground mt-3">
						Navigation tab is ready. Workflow management functionality is intentionally not implemented yet.
					</p>
				</div>
			</section>
			<Footer />
